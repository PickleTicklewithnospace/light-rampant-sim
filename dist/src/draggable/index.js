...
function onMouseDown(e) {
    e.preventDefault();
    if (e.target === null || !targetHasId(e.target))
        return;
    ...
    let i = 1;
    for (let div of allDraggables)
        if (div.id !== clickedDiv)
            div.style.zIndex = (i++).toString(); // put all other divs behind the selected one
}
